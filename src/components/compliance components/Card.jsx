...
const Card = ({ data = {} }) => {
  ...
  // Applied font-['Calibri',_sans-serif] to the entire component scope
  const fontStyle = "font-['Calibri',_sans-serif]";

  if (entries.length === 0) {
    return (
      <div className={`w-full p-8 sm:p-10 text-center bg-slate-50 rounded-2xl border-2 border-dashed border-slate-200 animate-pulse ${fontStyle}`}>
  ...
  return (
    /* Responsive Grid */
    <div className={`grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6 ${fontStyle}`}>
