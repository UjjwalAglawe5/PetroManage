    ...
    >
      ...
      <div className="flex flex-col md:flex-row items-center mb-6 sm:mb-10 relative gap-4">
        <button
          ...
          className="w-full md:w-auto md:absolute left-0 flex items-center justify-center gap-2 px-5 py-3 text-[15px] font-bold text-white bg-slate-900 rounded-xl hover:bg-slate-800 transition-all shadow-lg cursor-pointer z-10"
          style={calibriStyle}
        >
        ...
        <div className="flex-1 text-center">
          <h1 className="text-xl sm:text-4xl font-black text-slate-900 inline-flex items-center gap-3">
            <FaHistory className="text-emerald-500 hidden sm:inline size-9" /> Audit History
          </h1>
          <p className="text-slate-500 text-[13px] font-black uppercase tracking-[0.2em] mt-1">
        ...
        <div className="flex-1 w-full lg:max-w-sm">
          <div className="relative">
            ...
            <input
              ...
              className="h-12 w-full pl-11 pr-4 bg-slate-50 border border-slate-100 rounded-2xl text-[15px] font-bold outline-none focus:ring-4 focus:ring-emerald-50 focus:border-emerald-500 transition-all"
              style={calibriStyle}
            />
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-center gap-3 w-full lg:w-auto">
          ...
          <div className="flex items-center justify-between gap-2 bg-slate-50 border border-slate-100 rounded-2xl px-4 h-12 w-full sm:w-auto">
            ...
            {(startDate || endDate) && (
              <button
                onClick={clearDates}
                className="cursor-pointer ml-1 p-1.5 hover:bg-white rounded-full text-slate-400 hover:text-red-500 transition-colors"
              >
                <FaTimes size={15} />
              </button>
            )}
          ...
          <div className="relative h-12 w-full sm:w-auto" ref={dropdownRef}>
            <button
              ...
              className="w-full h-full px-6 bg-slate-900 text-white text-[12px] font-black rounded-2xl hover:bg-slate-800 transition-all shadow-lg flex items-center justify-center gap-3 uppercase cursor-pointer"
              style={calibriStyle}
            >
                ...
                >
                  {['json', 'excel', 'pdf'].map((opt) => (
                    <button
                      ...
                      className="w-full px-5 py-4 text-left text-[13px] font-black text-slate-600 hover:bg-slate-50 flex items-center gap-3 border-b border-slate-50 last:border-0 cursor-pointer transition-colors uppercase"
                      style={calibriStyle}
                    >
                      Save as {opt}
                    </button>
        ...
        <div className="overflow-x-auto scrollbar-hide">
          <table className="w-full text-left border-collapse min-w-[800px] table-fixed">
            <thead className="bg-slate-50/50 border-b border-slate-100">
              <tr className="text-[15px] uppercase tracking-[0.15em] text-slate-400 font-black">
                ...
                currentItems.map((log, i) => (
                  <tr key={log.id || i} className="hover:bg-slate-50/50 transition-colors h-20">
                    ...
                    <td className="px-4 py-4 text-center">
                      <p className="text-slate-400 italic text-[13px] truncate px-2" title={log.oldValue}>{log.oldValue || "—"}</p>
                    </td>
                    <td className="px-4 py-4 text-center">
                      <p className="text-slate-700 font-bold text-[13px] truncate px-2" title={log.newValue}>{log.newValue || "—"}</p>
                    </td>
                    <td className="px-4 py-4 text-center text-slate-400 text-[13px] tabular-nums">{log.timestamp || "—"}</td>
                  </tr>
                ))
              ) : (
                <tr>
